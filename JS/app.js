...
$(document).ready(function () {
        ...
        if (e) {
            for(var i = 0; i < e.length; i++){
                if(e[i].value > 0.49){
                    ...
                    if(e[i].emotion === "happy" || e[i].emotion === "suprised"){
                        $(message).addClass("alert-success");
                        $(message).html("Satisfied");
                    } else if(e[i].emotion === "sad" || e[i].emotion === "angry"){
                        $(message).addClass("alert-danger");
                        $(message).html("Unsatisfied");
                    } else {
                        $(message).addClass("alert-info");
                        $(message).html("Neutral");
                    }
    ...
    document.addEventListener("clmtrackrIteration", function (t) {
        stats.update()
    }, !1);

    $("#show-capture").click(function () {
    ...
    $("#startbutton").click(function(){
        ...
        // op.prepend(img);
        op.html(img);
        $(".description-wrapper").removeClass("text-center");
        $(".output-wrapper").show();
